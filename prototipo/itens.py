...
class Itens:
    def __init__(self, largura_tela, altura_tela, tela_jogo, posicao_gera_cano):
        ...
        self.__x = 0
        self.__y = 300
        self.distancia_do_cano = 40

    @property
    def x(self):
        return self.__x

    def posicaoTela(self): #Sorteia uma posicão que o item vai ficar na tela

        range_inicio = self.largura_tela + self.distancia_do_cano
        range_final = range_inicio + \
                    (self.largura_tela - (self.posicao_gera_cano + self.distancia_do_cano))

        self.__x = random.randint(range_inicio, range_final)

    def geraCano(self): #Desenha o item na tela do jogo
